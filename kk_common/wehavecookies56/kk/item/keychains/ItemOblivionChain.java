...
public class ItemOblivionChain extends ItemKingdomKeys{
    ...
    if (ItemOblivion.keyPressed)
    {
    	ItemOblivion.keyPressed = false;
            if (player.getHeldItem() != null && player.getHeldItem().itemID == AddedItems.K114c.itemID)
            {
      player.inventory.setInventorySlotContents(player.inventory.currentItem, new ItemStack(AddedItems.K114));
    }
    }
    }
}